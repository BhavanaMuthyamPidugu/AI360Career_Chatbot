...
export async function synthesizeAnswer(query, snippets, profile) {
  // Build the structured text locally first
  const localText = buildStructuredAnswer(query, snippets, profile); // your existing helper

  const key = process.env.GOOGLE_API_KEY;
  if (!key) {
    console.warn("⚠️ GOOGLE_API_KEY missing, returning local structured answer.");
    return localText; // ← No Gemini call in this branch
  }

  // Only if key exists, polish with Gemini
  ...
  // SDK expects a string or parts; string shorthand is simplest
  ...
  const out = resp?.response?.text?.() ?? resp?.response?.candidates?.[0]?.content?.parts?.[0]?.text ?? localText;
  return out;
}
